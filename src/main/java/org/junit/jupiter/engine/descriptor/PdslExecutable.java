...
public class PdslExecutable {
     ...
     * <p>
     * A runtime exception will be thrown if any failures are uncounted by the test.
     */
    public MetadataTestRunResults execute() {
        MetadataTestRunResults results;
        if (pdslTest.isPresent()) {
            results = visitor.isPresent()
                    ? executor.runTestsWithMetadata(List.of(pdslTest.get()), visitor.get().get(), context)
    ...
    public Optional<SharedTestCase> getSharedTestCase() {
